...
description: simple & advanced loading patterns in React Three Fiber
---
...
After using R3F (React Three Fiber) in production for over 2 years, I have learned a lot about loading patterns.
...
By the end of this blog, you will have a deep understanding on how loading assets in R3F works, and how to use them to improve the user experience and performance of your applications.
...
- Very basic knowledge of Three.js
- Very basic knowledge of R3F
- Very basic knowledge of React
...
In React, components re-render when their props or state change. This is a fundamental concept in React and is what makes it both powerful and tricky.

Because we are using React components, the rules of re-renders apply to us as well.
This means that every time the `MyMesh` Component re-renders, `TextureLoader` will be called again.
...
Luckily for us, R3F provides us all the tools we need to load textures correctly.
...
<Callout type="info">
  ...
  This is how React works and is required to display the new scaled mesh.
</Callout>
...
Congratulations! 🥳 You just finished learning the correct method to load assets in React Three Fiber!
Would you believe me if I told you that this is just the start? 🤯
...
`Suspense` in its simplest form allows us to show a fallback UI while we are waiting for data to load.
...
In case of React Three Fiber, we cannot display a fallback UI while loading an asset, because rendering HTML elements inside of a `Canvas` is not possible.

But that doesn't mean that `Suspense` cannot be useful for us. Everytime `useLoader` loads an **uncached** asset, it will throw a promise, which suspends the application.
...
Remember the `Suspense` inside the Canvas? It is the reason why this happens. As we learned before, the `Suspense` unmounts its children until the loading is done.
...
The key takeaway from this is that `Suspense` is a powerful tool that allows us to control the loading states of our assets inside the `Canvas`. How you use it is up to you, a few examples:

- Group assets together and only display them when they all loaded.
- Display a fallback mesh while the asset is loading.

Unfortunately, `Suspense` is not a perfect solution for all usecases. Lets dive into some more advanced loading patterns where its problems become more clear, and how we can solve them.
...
Click the cube a few times and notice the problem. The cube disappears while the new image is being fetched.
This happens because useLoader doesn't only throw a promise when the first texture is loaded. Each uncached texture will throw a promise.

If you click a few more times until the cycle restarts, you notice that the problem does no longer occur. The textures are now cached and can be displayed instantly.
...
That's why I personally never use `useTransition`, because `useDeferredValue` allows us to achieve the same result with way less code and complexity.
...
<Callout type="info">
  `useDefferedValue` is an advanced React hook but is very easy to use in
  combination with `useLoader`. Checkout [the
  docs](https://react.dev/reference/react/useDeferredValue) for more info.
</Callout>

`useDeferredValue` allows us to defer the update of our texture, until the new texture is loaded. This means that we keep displaying the old texture while we wait for `useLoader` to finish. This is exactly what we need to solve our problem of flickering textures.
...
Need the `isPending` we had with `useTransition`? Just comparing the current path and the deferred path. If they are not equal, we know that the texture is still loading.
...
When you need multiple assets of the same type inside one component, there is no need to use `useLoader` multiple times. You can pass an array of paths to `useLoader` and it will return an array of assets.
...
This works with preloading as well:
...
In this example, both `MaterialRepeat2` and `MaterialRepeat4` will have the same repeat value when rendered, depending on which one comes last in the component three.
...
Congratulations! 🥳 You just finished learning about loading patterns in React Three Fiber. I know it was a lot, and don't worry if not everything clicked from the first time.
Once you start using these patterns inside your applications, it will get easier to understand.

The next chapter takes us into the world of React Three Drei, where we will learn about hooks and components that make loading assets even easier, while still applying the patterns we just learned. Lets dive in!
...
Some info on the returned values:
...
Now a the text will be displayed in the center of our `Canvas`.

#### tunnel-rat

If you want even more control over your loading screen and how you display it, you can use a library called `tunnel-rat`.
You can use it to create 'tunnels' between the `Canvas` and R3F.
...
And then place the `t.Out` component outside of the `Canvas`, where ever you want to render it.
...
This is a very basic example, but I think you can imagine how powerful this library really can be. It allows you to create complex loading screens, overlays, ... from anywhere within your `Canvas`.\_createMdxContent

## The end

If you made it this far, congratulations! 🥳 I hope you learned something new and will impletement some of the things I showed in your applications!

If you have any questions, feedback or topics for future posts, feel free to reach out to me on [**X**](https://x.com/aaronclaes), I am always happy to hear from you!
