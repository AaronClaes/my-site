...
Congratulations! 🥳 You learned the correct method to load assets in React Three Fiber!
Would you believe me if I told you this is just the start? 🤯
...
These hooks are built on top of the `useLoader` hook, but take specific arguments relevant to the asset type. Let's take a look at a few examples.
...
When loading a texture, you often need to apply changes. It could be wrapping, repeating, anisotropy, etc. To make this more convenient, `useTexture` has a second argument that allows you to pass a callback function.

The callback function gives access to the loaded texture so you can apply changes.
...
Preloading & clearing with `useTexture` works exactly the same, but without the loader:
...
The `useGLTF` hook loads `.glb` and `.gltf` files. It takes care of Draco compression for you as well.
...
Not all are based on `useLoader`, so to check out the [Drei docs](https://github.com/pmndrs/drei?tab=readme-ov-file#loader) for more info.
