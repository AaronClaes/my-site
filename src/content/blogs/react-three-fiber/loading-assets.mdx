...
After using R3F (React Three Fiber) in production for over 2 years, I have learned a lot about loading assets, the best practices and the pitfalls it brings.
...
By the end of this blog, you will have a deep understanding on how loading assets in React & R3F works, and how to use it to improve the user experience and performance of your applications.
...
Lets take a look at what happens when we add a bit more functionality to our component:
...
But do not worry, React & React Three Fiber provides us all the tools we need to load assets correctly.

## The R3F way 👍

The maintainers of R3F were clearly aware of the re-render problems and provided us with built in solutions. Lets take a look how we can use them.
...
<Callout type="info">
  ...
  This is how React works and it is required to display the new scaled mesh if
  you modify it via props.
</Callout>
...
Later on in the blog we will take a look at some examples where this might cause unexpected issues, but for most usecases this is the behavior we want. It is important to remember that creating new instances of Three.js classes can be expensive, so reusing and caching them is great for performance.
...
In case of React Three Fiber, we cannot display a fallback UI while loading an asset, because rendering HTML elements inside of a three.js canvas is not possible. But that doesn't mean that `Suspense` cannot be useful for us.
...
So how can `Suspense` be useful to us? Lets take a look.
...
Great right? No flickering textures & everything is loaded before we start rendering. But what if we want to load new assets later on, when a element is clicked for example?

If you used `Supsense` before, you might already know where I am going with this, lets take a look.
...
The moment you click on the cube, the texture starts loading and everything dissapears for a brief moment, until the loading is finished.
...
Because the promise thrown by `useLoader` is caught by the first `Suspense` up the component tree (our `Suspense`), the problem is now fixed.

<Callout type="warn">
  The `Suspense` needs to be outside of the component that throws the promise.
  In this case that is the `ThreeCube` component with the `useLoader`.
</Callout>

<SandpackUseLoaderOnClickSuspense />

Click the cube and notice that `ThreeCube` appears when the texture is finished loading, while the `ReactCube` stays visible.
...
It is something that can slow down the loading time of your applications by a lot. `Suspense` can be great to prevent this, but only if you use it carefully and correctly. Let's learn how.
...
Both `MyMesh` components will throw a promise, which will be caught by the `Suspense` component. This will cause both textures to be loaded in parallel. The reason this works is because `Suspense` does not stop looking for promises once it found one. It also checks the remaining children to see if another promise is present.
...
You have to either structure your component similar to the first example we looked at, or make use of the solution built in `useLoader`.
...
<Callout type="info">
  ...
  throttle your network and see how your assets are loading. I am sure you will
  notice some things you did not expect.
</Callout>

Unfortunately, `Suspense` on its own is not a perfect solution for all usecases. Lets dive into some more advanced loading strategies where its problems become more clear, and how we can solve them.
...
This happens because `useLoader` doesn't only throw a promise when the first texture loads. Each uncached texture will throw a promise. Which causes the `Suspense` to unmount its children.
...
The main loading strategies are now covered. You learned how to load assets correctly, how to handle loading states and how to preload assets. But we are not done yet.
...
Congratulations! 🥳 You just finished learning about loading strategies in React Three Fiber. I know it was a lot, and don't worry if not everything clicked from the first time.
...
Each type of asset requires a different strategy to load. The `useLoader` hook is fine, but it is not always the best solution out of the box. Drei provides us with custom hooks for the most common asset types.
...
The `Html` component allows you to render HTML elements from within the `Canvas`. This is perfect for creating a custom loading screen.
...
If you made it this far, congratulations! 🥳 I hope you learned something new and will impletement it into your applications!
