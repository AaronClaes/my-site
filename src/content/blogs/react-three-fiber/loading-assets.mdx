...
But why would we benefit from this? Three.js by default uploads the texture to the GPU when it is visible for the renderer. Which means that when the texture is behind the camera, it will not compile until it is actually seen by the camera.
...
Not all hooks are based on `useLoader`, so make sure to check out the [Drei docs](https://github.com/pmndrs/drei?tab=readme-ov-file#loader) for more info.
